...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_weight = attn.sum(-2)  # 64*6*216
        # 取top167
        top_num = (N - 20) // 4 * 3
        bv, bi = torch.topk(attn_weight, top_num)
        threshold = bv[:, :, top_num - 1]
        ones = torch.ones_like(attn_weight)
        threshold = threshold.unsqueeze(2).expand_as(ones)
        mask = torch.gt(attn_weight, threshold)
        mask = mask.repeat(1, 1, N).reshape(B, 6, N, N)
        attn = attn * mask
