...
def put_palette_coco(seg_label, out_name):
    out = Image.fromarray(seg_label.astype(np.uint8), mode="P")
    colormap = imgviz.label_colormap()
    out.putpalette(colormap.flatten())
    out.save(out_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--bg_thr", default=0.06, type=float, help='the optimal one obtained for seeds')
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        cam_full_arr = np.zeros((21, orig_shape[2], orig_shape[3]), np.float32)   # pascal voc
        # cam_full_arr = np.zeros((81, orig_shape[2], orig_shape[3]), np.float32)     # coco
        ...
        with torch.no_grad():
            if dwidth < 10:
                all_zero = torch.zeros(orig_shape[2], orig_shape[3])
                res = np.uint8(all_zero.cpu())
                put_palette_coco(res, os.path.join(args.out_rw, name + '.png'))
                continue
            ...
            cam_vec = cam_full_arr.view(21, -1)  # pascal voc
            # cam_vec = cam_full_arr.view(81, -1)  # coco

            cam_rw = torch.matmul(cam_vec.cuda(), trans_mat)
            cam_rw = cam_rw.view(1, 21, dheight, dwidth)    # pascal voc
            # cam_rw = cam_rw.view(1, 81, dheight, dwidth)        #coco
            ...
            put_palette(res, os.path.join(args.out_rw, name + '.png'))  # pascal voc
            put_palette_coco(res, os.path.join(args.out_rw, name + '.png'))  # coco