...
def get_img_path(img_name, voc12_root):
    # print(f'77777777777: {img_name}')
    # print(f'77777777777: {voc12_root}')
    if img_name.find('COCO') != -1:
        return os.path.join(voc12_root, 'train2014', img_name + '.jpg')
        # print('77777777777')
    else:
...
def load_img_name_list(dataset_path):
    ...
    if dataset_path.find('coco') != -1:
        img_gt_name_list = open(dataset_path).read().splitlines()
        # img_name_list = [img_gt_name.split(' ')[0][-27:-0] for img_gt_name in img_gt_name_list]
